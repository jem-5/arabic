import React from "react";
export default function Contact() {
  return (
    <main className="flex-grow flex flex-col items-left p-3 text-neutral w-1/2 bg-[white] rounded-md mt-2 drop-shadow-xl border gap-3 max-[999px]:w-4/5">
      <h3 className="font-bold text-xl  self-center ">Send a Message</h3>
      <div className="divider "></div>
      <p>
        I`&lsquo;`m here to help you on your journey to learning Arabic. Whether
        you have questions, feedback or need assistance with our platform, feel
        free to reach out. Your experience with ArabicRoad is important to me,
        and I`&lsquo;`m committed to providing the support you need.
      </p>
      <p>
        You can contact me via email at jenniferengineers[at]gmail[dot]com. I
        strive to respond to all inquiries within 24-48 hours.
      </p>
      <p>
        Thank you for being a part of the ArabicRoad community. I look forward
        to hearing from you!
      </p>
    </main>
  );
}
